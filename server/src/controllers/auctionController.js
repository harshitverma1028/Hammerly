...
exports.getAuctionByItem = asyncHandler(async (req, res) => {
  ...
  if (!auction) {
    return res.status(404).json({ success: false, message: "No auction found for this item" });
  }
...
exports.createAuction = asyncHandler(async (req, res) => {
  const { item, startTime, endTime, startingBid, bidIncrement } = req.body;

  if (!item || !startTime || !endTime || !startingBid || !bidIncrement) {
  ...
  const existingActive = await Auction.findOne({ item, status: { $in: ["upcoming", "live"] } });
  if (existingActive) {
    return res.status(400).json({ success: false, message: "This item already has an active auction" });
  }
  ...
  const now = new Date();
  const status = new Date(startTime) <= now ? "live" : "upcoming";
...
exports.updateAuction = asyncHandler(async (req, res) => {
  ...
  if (auction.totalBids > 0) {
    return res.status(400).json({ success: false, message: "Cannot edit an auction that already has bids" });
  }

  const fields = ["startTime", "endTime", "startingBid", "bidIncrement"];
  fields.forEach((field) => {
    if (req.body[field] !== undefined) auction[field] = req.body[field];
  });

  if (req.body.startingBid !== undefined) {
    auction.currentBid = req.body.startingBid;
  }
